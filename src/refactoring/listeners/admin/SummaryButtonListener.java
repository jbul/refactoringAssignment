...
public class SummaryButtonListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		parent.f = new JFrame("Summary of Transactions");
		parent.f.setSize(400, 700);
														...
														// transaction.
		{
			for (int b = 0; b < parent.customerList.get(a).getAccounts().size(); b++) {
				parent.acc = parent.customerList.get(a).getAccounts().get(b);
				for (int c = 0; c < parent.customerList.get(a).getAccounts().get(b).getTransactionList().size(); c++) {

					textArea.append(parent.acc.getTransactionList().get(c).toString());
